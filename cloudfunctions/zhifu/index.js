...
exports.main = async (event, context) => {
  ...
  if(event.isQuery){
    const res = await cloud.cloudPay.queryOrder({
      "sub_mch_id": "1601026104",//这里要注意：虽然key是子商户id，实际上就是普通商户id
    ...
    const res = await cloud.cloudPay.unifiedOrder({
      ...
      "totalFee": Math.round(parseFloat(event.money)*100),//第二个坑：注意必须是数字，如果不是数字，则会报错unifiedOrder:fail wx api error: -202
